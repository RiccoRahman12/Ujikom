<!DOCTYPE html>
<html lang="en">

<head>

  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="">
  <meta name="author" content="">

  <title></title>

  <!-- Custom fonts for this template-->
  <link href="vendor/fontawesome-free/css/all.min.css" rel="stylesheet" type="text/css">
  <link href="https://fonts.googleapis.com/css?family=Nunito:200,200i,300,300i,400,400i,600,600i,700,700i,800,800i,900,900i" rel="stylesheet">

  <!-- Custom styles for this template-->
  <link href="css/sb-admin-2.min.css" rel="stylesheet">

</head>

<body id="page-top">

  <div class="card shadow">
    <div class="card-header">
      <span>Form Pengaduan</span>
    </div>
    <div class="card-body">
      <form action="simpan_p.php" method="post" class="form-horizontal" enctype="multipart/form-data">
        <div class="form-group">
          <span>Tanggal Pengaduan</span>
          <input type="text" name="tgl_pengaduan" value="<?php echo date('Y/m/d'); ?>" class='form-control' readonly>
        </div>
        <div class="form-group">
          <span>NIK</span>
          <input type="text" name="nik" value="<?php echo $_SESSION['nik']; ?>" class="form-control" readonly>
        </div>
        <div class="form-group">
          <span>Nama</span>
          <input type="text" name="nama" placeholder="Masukan nama anda" class="form-control" >
        </div>
        <div class="form-group">
          <span>Isi Laporan</span>
          <textarea name="isi_laporan" class="form-control" rows="5" required></textarea>
        </div>
        <div class="form-group">
          <span>Unggah Foto</span>
          <input type="file" name="foto" class="form-control" accept=".jpg, .png, .gif" required>
          <span class="text-danger">*Ekstensi yang diperbolehkan : .jpg, .gif, .png</span>
        </div>
        <div class="form-group">
          <input type="submit" name="submit" value="Simpan" class="btn btn-success">
          <input type="reset" name="reset" value="Kosongkan" class="btn btn-danger">
        </div>
      </form>
    </div>
  </div>

<!-- Bootstrap core JavaScript-->
  <script src="vendor/jquery/jquery.min.js"></script>
  <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>

  <!-- Core plugin JavaScript-->
  <script src="vendor/jquery-easing/jquery.easing.min.js"></script>

  <!-- Custom scripts for all pages-->
  <script src="js/sb-admin-2.min.js"></script>

</body>

</html>